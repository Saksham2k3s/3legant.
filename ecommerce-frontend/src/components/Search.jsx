  ...
  const getSearchProducts = async (query) => {
    try {
      const response = await axios.get(
        `${REACT_APP_PRODUCT_API_URL}/all?keyword=${query}`
      );
    ...
    <>
      <div className="absolute border-t-2 border-black h-[70%] bottom-0 w-full bg-white rounded-t-lg z-50 py-5 px-3">
        ...
        {searchedProducts.length > 0 && (
          <div className="flex flex-col w-full justify-center gap-4 px-5 py-1">
            <div className="w-[100%] lg:w-[50%] self-center ">
              {searchedProducts.map((product) => {
                return (
                  <div
                    className="w-full flex gap-4 justify-start align-middle px-2 py-4 cursor-pointer hover:bg-slate-300"
                    key={product._id}
                    onClick={() =>
                      handleNavigation(product.category, product._id)
                    }
                  >
                    <div>
                      <img
